...
from scapy.layers.tls.handshake import TLS13ClientHello, TLSClientHello
...
parser.add_argument(
    "--ticket_in", dest="session_ticket_file_in", help="File to read a ticket from (for TLS 1.3)"
)
parser.add_argument(
    "--ticket_out", dest="session_ticket_file_out", help="File to write a ticket to (for TLS 1.3)"
)
...
parser.add_argument("server", nargs="?", default="127.0.0.1", help="The server to connect to")
parser.add_argument("port", nargs="?", type=int, default=4433, help="The TCP destination port")
