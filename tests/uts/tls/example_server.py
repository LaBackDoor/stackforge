...
parser.add_argument(
    "--cert", default=scapy_path("/test/scapy/layers/tls/pki/srv_cert.pem"), help="Cert file."
)
parser.add_argument(
    "--key", default=scapy_path("/test/scapy/layers/tls/pki/srv_key.pem"), help="Key file."
)
parser.add_argument("--psk", help="External PSK for symmetric authentication (for TLS 1.3)")  # noqa: E501
parser.add_argument("--no_pfs", action="store_true", help="Disable (EC)DHE exchange with PFS")
parser.add_argument("--pcs", help="Preferred Cipher Suite (ex: 0x1301 = TLS_AES_128_GCM_SHA256)")
parser.add_argument("--psa", help="Preferred Signature Algorithm (ex: sha256+rsaepss)")
...
parser.add_argument(
    "--cookie", action="store_true", help="Send cookie extension in HelloRetryRequest message"
)
parser.add_argument("--client_auth", action="store_true", help="Require client authentication")
parser.add_argument(
    "--handle_session_ticket",
    action="store_true",
    help="Use session tickets. Auto enabled if file provided (for TLS 1.3)",
)  # noqa: E501
parser.add_argument(
    "--ticket_file", dest="session_ticket_file", help="File to write/read a ticket to (for TLS 1.3)"
)
parser.add_argument("--debug", action="store_const", const=5, default=0, help="Enter debug mode")
